...
registerLocation({
  ...
  ru: 'Арбат',
  kk: 'Арбат',
  // Внутри улицы, а не под аркой.
  //
  // Появление на z = 8 ставило камеру, которая тянется метров на девять назад,
  // за ворота на z = 10 — и вывеска арки шириной девятнадцать метров занимала
  // верхнюю треть кадра. Первое, что ребёнок видел, открыв «Город», была
  // коричневая стена, а сам Арбат — узкой полосой под ней.
  //
  // Отсюда арка остаётся позади и читается как вход, через который только что
  // прошли: её видно, если повернуть камеру.
  spawn: { x: 0, z: -2 },
...
registerLocation({
  ...
  // Внутри парка, а не на его западной кромке.
  //
  // Появление на x = −46 стояло в четырёх метрах от портала на самом краю:
  // геометрия парка там кончается, и треть кадра занимала пустота за краем
  // земли. Двенадцать метров вглубь — и вокруг парк со всех сторон, а портал
  // остаётся позади, куда и уходят.
  spawn: { x: -38, z: 0 },
