...
| `0bc6a73` | Local renderer telemetry | FPS-only logs could hide draw-call, triangle, geometry and texture regressions from new art. Dev QA can now opt into one quiet whole-frame sample with `?perf=1`, without a player-facing overlay or production code. | Type-check, lint, build; production bundle excludes the probe; L0 390×844 / 1440×900, City and wardrobe-preview smokes reported metrics with no console errors. See ADR 007. |
