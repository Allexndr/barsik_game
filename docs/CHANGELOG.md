...
- **Deployment:** `dpl_5s2XZchMW9PSQYvvjsSVafaiYpwQ` →
  `https://barsik-game-fmuvg7nm2-allexndrs-projects.vercel.app` →
  `https://barsik-game.vercel.app`.
- **Release gate:** integrated type-check, lint and production build passed;
  the production bundle excludes the local `?perf=1` probe. Fresh local
  phone/desktop City first-action and Town-diorama checks had no console
  errors.
- **Rollback:** previous ready production
  `dpl_2GWxqsfUbGEWu6Td66ijXJiBHa7t`; use Vercel deployment history rather
  than an arbitrary preview if an immediate rollback is required.

### Production deployment — 2026-08-11 14:07 Asia/Almaty

- **Source:** integration commit `70b3082`.
- **Deployment:** `dpl_2GWxqsfUbGEWu6Td66ijXJiBHa7t` →
  `https://barsik-game-4p18fk2q3-allexndrs-projects.vercel.app` →
  `https://barsik-game.vercel.app`.
- **Release gate:** integrated type-check, lint and production build passed;
  the production bundle excludes the local `?perf=1` probe. Fresh local
  mobile/desktop Winter first-frame and Forest grass smokes had no console
  errors.
- **Rollback:** previous ready production
  `dpl_HX6chqdsF2ZdaXN2dYh3PSQHG35o`; use Vercel deployment history rather
  than an arbitrary preview if an immediate rollback is required.
...
| `219881d` | City Kit foundation | The City’s first 3D image was a prototype field of primitive houses and trees, with stale async decoration able to arrive after a rebuild. The permanent town now uses cached, deterministic lightweight CC0 buildings, path, trees and planters with generation-safe ownership. | Type-check, lint, build; City zero-friend phone/desktop, real resident picking, forced rebuild and delayed-dispose smoke. 0-friend local renderer sample: ~146–147 calls and ~36.5k triangles. See ADR 013. |
| `68ae281` | City first-friend path | An empty City had no named action in its first viewport; language and outfit changes could rebuild the entire scene; the close target was too small and legacy violet styling remained. The City now has an in-view Journey CTA, incremental outfit update, language-only copy update and touch-safe controls in the locked palette. | Type-check, lint, build; integrated 390×844 + 1440×900 empty-City CTA flow with no errors; RU→KK resident update with no new GLB resource entries. See ADR 014. |
| `5504710` | Winter snow/ice surface kit | Winter terrain was largely a pale vertex-colour plane and L12’s ice trail was an untextured grey surface. Winter levels now use small seeded packed-snow/ice material detail and L12 has an authored icy ribbon, with no draw-call or geometry increase. | Type-check, lint, build; integrated 390×844 L11/L12/L14/L16 and 1440×900 L12 live checks without console errors; L13 mobile renderer sample 178,164 triangles. See ADR 015. L12’s pre-existing normal-phone triangle breach remains recorded, not waived. |
| `1ef0ab3` | Season 1 five-minute blueprint | Existing missions needed one canonical, implementation-ready story and pacing reference rather than stale or contradictory level notes. The blueprint reconciles live L0 with S1 planning and gives every level an explicit child-safe five-beat pass. | `git diff --check` and `git show --check`; document-only, no runtime claim. See `S1_GAMEPLAY_5MINUTE_IMPLEMENTATION.md`. |
